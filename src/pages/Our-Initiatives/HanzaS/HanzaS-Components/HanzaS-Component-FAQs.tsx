...
const HanzaSComponentFAQs: React.FC = () => {
  return (
    <motion.section
      className="snap-start min-h-screen w-full bg-gray-800 px-4 sm:px-6 py-24 flex flex-col items-center justify-center relative"
      initial={{ opacity: 0 }}
    ...
    >
      {/* Header */}
      <motion.h2
        className="text-3xl sm:text-5xl font-bold font-serif text-white mb-16 text-center leading-snug"
        initial={{ opacity: 0, y: 30 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition={{ duration: 1 }}
      >
        FAQs — Your Journey, Answered
      </motion.h2>

      {/* FAQ Content */}
      <div className="relative border-l-2 border-purple-500/30 pl-6 max-w-4xl w-full space-y-10 sm:space-y-12">
        {faqs.map((faq, index) => (
...
>
  ...
  px] top-3 w-4 h-4">
    <div className="relative w-full h-full">
      ...
      <div className="relative w-4 h-4 rounded-full bg-purple-400 shadow-md z-10" />
    </div>
  </div>

  {/* FAQ Card */}
  <div className="bg-white/5 border border-purple-500/20 hover:border-purple-400/40 hover:shadow-xl transition-all duration-300 backdrop-blur-md rounded-xl p-5 sm:p-6 text-white">
    <h3 className="text-base sm:text-lg font-semibold mb-2 text-purple-200">
      {faq.title}
    </h3>
    <p className="text-sm sm:text-base text-gray-300 leading-relaxed">
      ...
      {/* CTA */}
      <motion.div
        className="mt-16 text-sm text-white text-center opacity-60"
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        transition={{ delay: 0.5 }}
      >
        Still curious?{" "}
        <a href="/contact-us" className="text-purple-400 underline hover:text-purple-200">
          Reach out to us
        </a>
      </motion.div>
    </motion.section>
