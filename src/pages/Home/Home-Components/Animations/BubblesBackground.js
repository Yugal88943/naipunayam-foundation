...
export const BubblesBackground = () => {
    ...
    useEffect(() => {
        function updateMask() {
            ...
            let introEl = document.getElementById("home-intro")
                || document.querySelector(".Home-Component-Intro")
    ...
    return (_jsxs(_Fragment, { children: [_jsx("style", { children: bubbleKeyframes }), _jsx("div", { className: "fixed inset-0 w-full h-full z-0 pointer-events-none", "aria-hidden": true, style: {
                    overflow: "hidden",
                    ...
                    WebkitMaskImage: maskPx
                        ? `linear-gradient(to bottom, transparent 0px, transparent ${maskPx - 8}px, black ${maskPx + 24}px, black 100%)`
                        : undefined
                }, children: BUBBLES.map((bubble, idx) => (_jsx("span", { style: {
                        position: "absolute",
                        top: bubble.top,
                        left: bubble.left,
                        width: bubble.size,
