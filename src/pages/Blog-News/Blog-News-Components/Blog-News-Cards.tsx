...
const BlogNewsCards: React.FC = () => {
  return (
    ...
    <section className="w-full max-w-7xl mx-auto px-4 py-10 bg-gradient-to-br from-white to-gray-50">
          ...
          >
            ...
            <div className="md:w-1/3 flex-shrink-0 flex items-center justify-center overflow-hidden bg-white/30">
              <motion.img
                src={post.image}
                alt={post.title}
                className="w-full h-48 md:h-44 object-cover object-center rounded-t-3xl md:rounded-tr-none md:rounded-l-3xl group-hover:scale-105 group-hover:brightness-105 transition-transform duration-400"
                loading="lazy"
                draggable={false}
      ...
      {/* CSS keyframes for button shimmer */}
      <style>
        {`
          .shimmer-bg {
